...
The remaining input is now `AB`, which we see exists in the prefix tree, so the encoder outputs code 3. Since there is no more input, no string is added to the prefix tree at this step.
